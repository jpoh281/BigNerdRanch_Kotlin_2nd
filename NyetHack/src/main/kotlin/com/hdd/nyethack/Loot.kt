    ...
    companion object {
        fun random(): LootBox<Loot> = LootBox(
            contents = when (Random.nextInt(1 .. 100)) {
                ...
                in 6 .. 10 -> Fedora("fedora of knoledge", 125)
                in 11 .. 15 -> Fedora("stunning teal fedora", 75)
                in 16 .. 30 -> Fez("ordinary fedora", 15)
...
class Fedora(override val name: String, override val value: Int) : Hat()

class Fez(override val name: String, override val value: Int) : Hat()

class Gemstones(override val value: Int) : Loot(), Sellable {
    override val name: String = "sack of gemstones worth $value gold"
}
